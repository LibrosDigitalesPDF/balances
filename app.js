...
const LOADER_PHASES = [
    ...
    "Sincronizando configuración de Drive...",
...
function initTheme() {
    ...
    document.getElementById('theme-toggle').addEventListener('click', () => {
        ...
        const isDark = document.body.classList.contains('dark-mode'); localStorage.setItem('theme', isDark ? 'dark' : 'light');
...
function initNavModules() {
    const navBtns = document.querySelectorAll(".top-nav-btn"); const modules = document.querySelectorAll(".app-module");
    navBtns.forEach(btn => {
...
function setupEventListeners() {
    ...
    document.getElementById("btn-add-tab-sheet").addEventListener("click", () => {
        ...
        if (newTab.trim().toLowerCase() === "ingresos" || newTab.trim().toLowerCase() === "carpetas") { alert("Nombre reservado del sistema."); return; }
    ...
    document.getElementById("btn-save-carpetas").addEventListener("click", () => {
        const sheetName = appState.currentHistorySheet; const cData = appState.carpetas[sheetName] || { rowIndex: null };
        const payload = { rowIndex: cData.rowIndex, sheetName: sheetName, idCuenta: document.getElementById("edit-id-cuenta").value.trim(), idCompra: document.getElementById("edit-id-compra").value.trim(), idPago: document.getElementById("edit-id-pago").value.trim() };
    ...
    // UPLOAD DIRECTO (La web ya no calcula folders, lo hace el backend)
    document.getElementById("global-file-input").addEventListener("change", function(e) {
        ...
        reader.onload = function(evt) {
            ...
            const payload = { action: "UPLOAD_FILE", data: { sheetName: appState.currentUpload.sheetName, rowIndex: appState.currentUpload.rowIndex, type: appState.currentUpload.type, fileName: file.name, mimeType: file.type, fileBase64: base64String } };
...
function toggleLoader(show, msg = "") {
    ...
    let phaseIdx = 0; msgEl.textContent = LOADER_PHASES[phaseIdx];
    loaderInterval = setInterval(() => { phaseIdx = (phaseIdx + 1) % LOADER_PHASES.length; msgEl.textContent = LOADER_PHASES[phaseIdx]; }, 1100);
}
...
function fetchFinancialData() {
    ...
    fetch(API_URL).then(res => { if (!res.ok) throw new Error(); return res.json(); }).then(json => {
        if (json.status === "success") {
            appState.balances = json.data.balances; appState.carpetas = json.data.carpetas || {}; appState.proveedores = json.data.proveedores || []; appState.sueldos = json.data.sueldos || {};
            ...
            if (appState.currentHistorySheet) { renderHistoryTable(); renderCarpetasSection(appState.currentHistorySheet); } 
        }
...
function renderBalance() {
    ...
    Object.keys(appState.balances.gastos).sort().forEach(sheetName => {
        ...
        periodData.forEach(mov => { const absMonto = Math.abs(mov.monto); categoryTotal += absMonto; movimientosCount++; rowsHtml += `<tr><td class="text-left">${mov.fecha}</td><td class="text-left">${mov.detalle || "-"}</td><td class="text-left">${mov.operacion || "-"}</td><td class="text-right" style="font-weight:600; color: var(--text-primary);">${formatArgentineCurrency(absMonto)}</td></tr>`; });
        grandTotalGastos += categoryTotal;
        const accItem = document.createElement("div"); accItem.className = "accordion-item";
        accItem.innerHTML = `<div class="accordion-header"><div class="accordion-title-group"><svg class="accordion-icon" viewBox="0 0 24 24"><path fill="currentColor" d="M7.41,8.58L12,13.17L16.59,8.58L18,10L12,16L6,10L7.41,8.58Z" /></svg><span class="item-name">${sheetName}</span></div><span class="item-val">${formatArgentineCurrency(categoryTotal)}</span></div><div class="accordion-body"><div class="accordion-content" style="padding-top:0; padding-bottom:0; border:none;"><div class="table-responsive"><table class="detail-table" style="margin:0; border-radius:0;"><thead><tr><th class="text-left">Fecha</th><th class="text-left">Detalle</th><th class="text-left">Operación</th><th class="text-right">Monto</th></tr></thead><tbody>${rowsHtml}</tbody><tfoot><tr class="table-total-row"><td colspan="3" class="text-right">TOTAL PESTAÑA</td><td class="text-right" style="color:var(--text-primary);">${formatArgentineCurrency(categoryTotal)}</td></tr></tfoot></table></div></div></div>`;
    ...
    const ingresosList = document.getElementById("ingresos-list"); ingresosList.innerHTML = ""; const ingresosData = appState.balances.ingresos["Ingresos"]?.[periodKey];
...
function renderCarpetasSection(sheetName) {
    ...
    document.getElementById("view-id-cuenta").textContent = cData.idCuenta || "-"; document.getElementById("view-id-compra").textContent = cData.idCompra || "-"; document.getElementById("view-id-pago").textContent = cData.idPago || "-";
    document.getElementById("edit-id-cuenta").value = cData.idCuenta; document.getElementById("edit-id-compra").value = cData.idCompra; document.getElementById("edit-id-pago").value = cData.idPago;
...
function createBalanceRowHTML(mov, isEditing, sheetName, type) {
    ...
    if (isEditing) { tr.innerHTML = `<td><input type="text" class="edit-input i-fec" value="${mov.fecha || ''}" placeholder="DD/MM/AAAA"></td><td><input type="text" class="edit-input i-det" value="${mov.detalle || ''}"></td><td><input type="number" step="0.01" class="edit-input i-mon" value="${mov.monto || ''}"></td><td><input type="text" class="edit-input i-ope" value="${mov.operacion || ''}"></td><td><input type="text" class="edit-input i-i21" value="${mov.iva21 || ''}"></td><td><input type="text" class="edit-input i-i105" value="${mov.iva105 || ''}"></td><td><input type="text" class="edit-input i-icon" value="${mov.ivaCont || ''}"></td><td class="text-center">-</td><td class="text-center">-</td><td class="action-buttons sticky-col"><button class="action-btn btn-save" onclick="saveBalance(this, ${mov.rowIndex || 'null'}, '${sheetName}', '${type}')">Guardar</button><button class="action-btn btn-cancel" onclick="renderHistoryTable()">Cancelar</button></td>`; } 
    else {
        ...
        const compHTML_P = mov.idComprobantePago ? `<a href="https://drive.google.com/file/d/${mov.idComprobantePago}/view" target="_blank" class="action-btn btn-link" style="text-decoration:none; display:inline-block;">Ver</a>` : `<button class="action-btn btn-secondary" style="border:1px solid var(--primary-color); color:var(--primary-color);" onclick="window.triggerUpload('${sheetName}', ${mov.rowIndex}, 'pago', '${type}')">Subir</button>`;
        tr.innerHTML = `<td>${mov.fecha || '-'}</td><td>${mov.detalle || '-'}</td><td class="text-right" style="font-weight:600; color: var(--text-primary);">${montoFormat}</td><td>${mov.operacion || '-'}</td><td>${mov.iva21 || '-'}</td><td>${mov.iva105 || '-'}</td><td>${mov.ivaCont || '-'}</td><td class="text-center">${compHTML_C}</td><td class="text-center">${compHTML_P}</td><td class="action-buttons sticky-col"><button class="action-btn btn-edit" onclick="editBalance(${mov.rowIndex}, '${sheetName}', '${type}')">Editar</button><button class="action-btn btn-delete" onclick="deleteBalance(${mov.rowIndex}, '${sheetName}')">Borrar</button></td>`;
    } return tr;
}
function addBalanceEmptyRow(sheetName, type) { const tbody = document.getElementById("historial-tbody"); document.getElementById("historial-table").classList.remove("hidden"); document.getElementById("historial-empty").classList.add("hidden"); const emptyMov = { fecha: "", detalle: "", monto: "", operacion: "", iva21: "", iva105: "", ivaCont: "", idComprobanteCompra: "", idComprobantePago: "" }; tbody.insertBefore(createBalanceRowHTML(emptyMov, true, sheetName, type), tbody.firstChild); }
...
function sendSueldosPostRequest(action, dataObj) { toggleLoader(true, "Procesando trabajador..."); fetch(API_URL, { method: "POST", body: JSON.stringify({ action: action, data: dataObj }) }).then(res => res.json()).then(res => { if (res.status === "success") fetchFinancialData(); else toggleLoader(false); }).catch(() => toggleLoader(false)); }
