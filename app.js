        ...
        appState.sueldos[year].forEach(worker => {
            ...
            for (let m = 0; m < 12; m++) {
                ...
                if (nombre && nombre !== "-" && !isNaN(sueldoNumber) && sueldoNumber > 0) {
                    ...
                    const fechaLimpia = (fechaPagoRaw && fechaPagoRaw !== "-") ? fechaPagoRaw.toString().split(" ")[0] : `01/${monthStr}/${year}`;
...
function populateSidebarHistory() {
    ...
    btnI.className = "history-btn"; 
    btnI.textContent = "Ingresos";
...
function setupEventListeners() {
    ...
    document.getElementById("btn-add-tab-sheet").onclick = () => {
        const t = prompt("Nombre de la nueva cuenta:"); 
        if(!t || t.trim()==="") return;
    ...
    document.getElementById("global-file-input").onchange = function(e) {
        ...
        reader.onload = function(evt) {
            ...
            fetch(API_URL, { method: "POST", body: JSON.stringify(payload) })
                .then(r=>r.json())
                .then(d => { if(d.status==="success") fetchFinancialData(); else { alert("Error"); toggleLoader(false); } })
...
function createBalanceRowHTML(m, edit, s, t) {
    ...
    if (edit) { tr.innerHTML = `<td><input type="text" class="edit-input i-fec" value="${m.fecha||''}" placeholder="DD/MM/AAAA"></td><td><input type="text" class="edit-input i-det" value="${m.detalle||''}"></td><td><input type="number" step="0.01" class="edit-input i-mon" value="${m.monto||''}"></td><td><input type="text" class="edit-input i-ope" value="${m.operacion||''}" placeholder="Operación"></td><td><input type="text" class="edit-input i-i21" value="${m.iva21||''}"></td><td><input type="text" class="edit-input i-i105" value="${m.iva105||''}"></td><td><input type="text" class="edit-input i-icon" value="${m.ivaCont||''}"></td><td class="text-center">-</td><td class="text-center">-</td><td class="action-buttons sticky-col"><button class="action-btn btn-save" onclick="saveBalance(this, ${m.rowIndex||'null'}, '${s}', '${t}')">Guardar</button><button class="action-btn btn-cancel" onclick="renderHistoryTable()">Cancelar</button></td>`; } 
    else {
...
window.saveProveedor = function(btnElement, rowIndex) { const tr = btnElement.closest("tr"); let currentProv = {}; if (rowIndex) currentProv = appState.proveedores.find(p => p.rowIndex === rowIndex) || {}; const getVal = (selector, fallback) => { const input = tr.querySelector(selector); return input ? input.value : (fallback || ""); }; const payload = { rowIndex: rowIndex, proveedor: getVal(".i-prov", currentProv.proveedor), nombre: getVal(".i-nom", currentProv.nombre), direccion: getVal(".i-dir", currentProv.direccion), banco: getVal(".i-ban", currentProv.banco), alias: getVal(".i-ali", currentProv.alias), cbu: getVal(".i-cbu", currentProv.cbu), telefono: getVal(".i-tel", currentProv.telefono), mail: getVal(".i-mail", currentProv.mail), web: getVal(".i-web", currentProv.web) }; sendGlobalPostRequest(rowIndex ? "PROV_EDIT" : "PROV_ADD", payload); };
...
// ==========================================
// MÓDULO: SUELDOS (NUEVA DISTRIBUCIÓN REORDENADA)
// ==========================================
function renderSueldos() {
    ...
    activeWorkers.forEach((worker) => {
        const tr = document.createElement("tr"); 
        const rData = worker.rowData;
        const fechaPagoLimpia = (rData[offset+12] || '').toString().split(' ')[0]; // Pura fecha argentina, remueve hora si existiera
        ...
        if (appState.sueldosEditMode) {
            let rowHtml = `
                ...
                <td><input type="text" class="edit-input s-fp" value="${fechaPagoLimpia}" placeholder="DD/MM/AAAA"></td>
            ...
            let rowHtml = `
                ...
                <td class="text-left">${fechaPagoLimpia || '-'}</td>
