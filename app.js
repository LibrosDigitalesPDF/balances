...
const LOADER_PHASES = ["Conectando con el servidor...", "Descargando flujos de caja...", "Sincronizando configuración...", "Cargando módulos comerciales..."];

let appState = { balances: null, carpetas: {}, proveedores: [], sueldos: {}, selectedMonth: "", selectedYear: "", historyMonth: "ALL", historyYear: "ALL", sueldosMonth: "", sueldosYear: "", sueldosEditMode: false, currentHistorySheet: null, currentHistoryType: null, currentUpload: null, activeProvRowIndex: null, activeProvTab: "gen" };
let loaderInterval = null;

document.addEventListener("DOMContentLoaded", () => { initTheme(); initNavModules(); initSelectors(); initTabs(); setupEventListeners(); setupWebModalHandlers(); fetchFinancialData(); });
...
function setupEventListeners() {
    ...
    document.getElementById("btn-add-tab-sheet").onclick = () => {
        ...
        if(t.trim().toLowerCase()==="ingresos" || t.trim().toLowerCase()==="carpetas") { alert("Nombre reservado."); return; }
        sendGlobalPostRequest("BAL_ADD_TAB", { sheetName: t.trim() });
    };
    document.getElementById("btn-del-tab-sheet").onclick = () => {
        if(!appState.currentHistorySheet || appState.currentHistoryType==="ingresos") { alert("Seleccione un gasto."); return; }
    ...
    document.querySelectorAll("#module-proveedores .menu-btn").forEach(btn => {
        btn.onclick = () => {
            ...
            renderProveedores(); // Re-renderiza para aplicar clases a las filas
        };
    });
    ...
    document.getElementById("global-file-input").onchange = function(e) {
        ...
        toggleLoader(true, "Procesando archivo...");
...
function openHistoryView(s, t, b) {
    ...
    document.getElementById("btn-add-balance").classList.remove("hidden"); document.getElementById("btn-add-balance").onclick = () => addBalanceEmptyRow(s, t);
    ...
    renderCarpetasSection(s); renderHistoryTable();
}
...
function createBalanceRowHTML(m, edit, s, t) {
    ...
    if (edit) { tr.innerHTML = `<td><input type="text" class="edit-input i-fec" value="${m.fecha||''}" placeholder="DD/MM/AAAA"></td><td><input type="text" class="edit-input i-det" value="${m.detalle||''}"></td><td><input type="number" step="0.01" class="edit-input i-mon" value="${m.monto||''}"></td><td><input type="text" class="edit-input i-ope" value="${m.operacion||''}"></td><td><input type="text" class="edit-input i-i21" value="${m.iva21||''}"></td><td><input type="text" class="edit-input i-i105" value="${m.iva105||''}"></td><td><input type="text" class="edit-input i-icon" value="${m.ivaCont||''}"></td><td class="text-center">-</td><td class="text-center">-</td><td class="action-buttons sticky-col"><button class="action-btn btn-save" onclick="saveBalance(this, ${m.rowIndex||'null'}, '${s}', '${t}')">Guardar</button><button class="action-btn btn-cancel" onclick="renderHistoryTable()">Cancelar</button></td>`; } 
    else {
        const cc = m.idComprobanteCompra ? `<a href="https://drive.google.com/file/d/${m.idComprobanteCompra}/view" target="_blank" class="action-btn btn-link" style="text-decoration:none;">Ver</a>` : `<button class="action-btn btn-secondary" style="border:1px solid var(--primary-color); color:var(--primary-color);" onclick="window.triggerUpload('${s}', ${m.rowIndex}, 'compra', '${t}')">Subir</button>`;
        const cp = m.idComprobantePago ? `<a href="https://drive.google.com/file/d/${m.idComprobantePago}/view" target="_blank" class="action-btn btn-link" style="text-decoration:none;">Ver</a>` : `<button class="action-btn btn-secondary" style="border:1px solid var(--primary-color); color:var(--primary-color);" onclick="window.triggerUpload('${s}', ${m.rowIndex}, 'pago', '${t}')">Subir</button>`;
        tr.innerHTML = `<td>${m.fecha||'-'}</td><td>${m.detalle||'-'}</td><td class="text-right" style="font-weight:600; color:var(--text-primary);">${formatArgentineCurrency(t==='gastos'?Math.abs(m.monto):m.monto)}</td><td>${m.operacion||'-'}</td><td>${m.iva21||'-'}</td><td>${m.iva105||'-'}</td><td>${m.ivaCont||'-'}</td><td class="text-center">${cc}</td><td class="text-center">${cp}</td><td class="action-buttons sticky-col"><button class="action-btn btn-edit" onclick="editBalance(${m.rowIndex}, '${s}', '${t}')">Editar</button><button class="action-btn btn-delete" onclick="deleteBalance(${m.rowIndex}, '${s}')">Borrar</button></td>`;
    } return tr;
}
...
// ==========================================
// MÓDULO PROVEEDORES (NUEVO CON SUBPESTAÑAS)
// ==========================================
...
function createProvRowHTML(prov, isEditing) {
    ...
    // Asignar clases de visualización según la pestaña activa
...
window.editProveedor = function(rowIndex) {
    const prov = appState.proveedores.find(p => p.rowIndex === rowIndex); if (!prov) return;
    const tbody = document.getElementById("proveedores-tbody"); const rows = Array.from(tbody.querySelectorAll("tr"));
...
window.saveProveedor = function(btnElement, rowIndex) {
    ...
    // En caso de que se esté editando desde otra pestaña, necesitamos recuperar los valores ocultos
    // Los inputs (aún ocultos por CSS) mantendrán sus valores perfectamente
    const payload = {
        rowIndex: rowIndex,
        ...
        telefono: tr.querySelector(".i-tel").value,
        mail: tr.querySelector(".i-mail").value,
...
window.openWebModal = function(rowIndex) { 
    appState.activeProvRowIndex = rowIndex; const prov = appState.proveedores.find(p => p.rowIndex === rowIndex); 
    document.getElementById('modal-prov-name').textContent = prov.nombre || prov.proveedor || "Nuevo Proveedor"; 
    document.getElementById('web-view-mode').classList.remove('hidden'); document.getElementById('web-edit-mode').classList.add('hidden'); 
    document.getElementById('btn-modal-edit').classList.remove('hidden'); document.getElementById('btn-modal-save').classList.add('hidden'); document.getElementById('btn-modal-cancel').classList.add('hidden'); 
    const link = document.getElementById('modal-web-link'); const noWeb = document.getElementById('modal-no-web'); 
    if(prov.web && prov.web.trim() !== "") { link.href = prov.web.startsWith('http') ? prov.web : 'https://' + prov.web; link.classList.remove('hidden'); noWeb.classList.add('hidden'); document.getElementById('modal-web-input').value = prov.web; } else { link.classList.add('hidden'); noWeb.classList.remove('hidden'); document.getElementById('modal-web-input').value = ""; } 
